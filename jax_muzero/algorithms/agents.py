...
class Agent(object):
    ...
    def __init__(self, observation_space: gym.spaces.Box, action_space: gym.spaces.Discrete, num_bins: int,
                 ...
                 q_normalize_epsilon: float, child_select_epsilon: float, mcts_c3: float, mode: str):
    ...
    def _root_step(self, rng_key: chex.PRNGKey, params: Params, timesteps: ActorOutput, temperature: float,
        ...
        agent_out = jax.tree_map(lambda t: t.squeeze(axis=0), agent_out)
    ...
    def init_tree(self, rng_key: chex.PRNGKey, root: AgentOutput, is_eval: bool):
        ...
        noise = jax.random.dirichlet(
            rng_key, jnp.full((num_actions,), self._alpha))
        # Do not apply the Dirichlet noise during evaluation.
        exploration_prob = jax.lax.select(is_eval, 0., self._exploration_prob)
        root_prob = jax.nn.softmax(
            root.logits) * (1 - exploration_prob) + exploration_prob * noise
    ...
    def mcts(self, rng_key: chex.PRNGKey, params: Params, root: AgentOutput, is_eval: bool):
        ...
        def simulate(rng_key: chex.PRNGKey, tree: Tree):
            ...
            def _select_action(rng_key: chex.PRNGKey, t, q_mean, w_mean):
                ...
                q = (q - q_min) / jnp.maximum(q_max -
                                              q_min, self._q_normalize_epsilon)
                sigma = (sigma - q_min) / jnp.maximum(q_max -
                                                      q_min, self._q_normalize_epsilon)
                p = tree.prob[t]
            ...
            def _cond(loop_state):
                rng_key, p, a, q_mean, w_mean = loop_state
        ...
        def expand(tree: Tree, p, a, c):
            ...
            tree = tree._replace(
                ...
                reward_logits=tree.reward_logits.at[c].set(
                    model_out.reward_logits),
                reward=tree.reward.at[c].set(model_out.reward),
                value_logits=tree.value_logits.at[c].set(
