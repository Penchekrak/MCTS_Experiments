...
def generate_update_fn(agent: agents.Agent, opt_update, unroll_steps: int, td_steps: int, discount_factor: float,
    ...
    def loss(params: Params, target_params: Params, trajectory: ActorOutput, rng_key: chex.PRNGKey):
        ...
        random_action_mask = jnp.cumprod(1. -
        ...
        reward_loss = jnp.mean(_batch_categorical_cross_entropy(
        ...
        value_loss = jnp.mean(_batch_categorical_cross_entropy(
            value_logits_target, value_logits))
        logits = jnp.concatenate(
            [learner_root.logits[None], model_out.logits], axis=0)
        policy_loss = jnp.mean(
            _batch_categorical_cross_entropy(policy_target, logits))
        total_loss = reward_loss + value_coef * value_loss + policy_coef * policy_loss
        policy_target_entropy = jax.vmap(
            lambda p: distrax.Categorical(probs=p).entropy())(policy_target)
        log = {
    ...
    def batch_loss(params: Params, target_params: Params, trajectories: ActorOutput, rng_key: chex.PRNGKey):
        ...
        losses, log = jax.vmap(loss, (None, None, 0, 0))(
            params, target_params, trajectories, rng_keys)
        log.update({
    ...
    def update(rng_key: chex.PRNGKey, params: Params, target_params: Params, opt_state, trajectories: ActorOutput):
        grads, log = jax.grad(batch_loss, has_aux=True)(
            params, target_params, trajectories, rng_key)
...
class Experiment(tune.Trainable):
    def setup(self, config):
        ...
        self._evaluate_actor = actors.EvaluateActor(
            self._evaluate_envs, self._agent)
        ...
        self._params = jax.device_put_replicated(
            self._params, jax.local_devices())
        self._target_params = self._params
    ...
    def step(self):
        ...
        for _ in range(self._log_interval):
            ...
            self._trajectories = [
                self._replay_buffer.sample(
                    self._batch_size // self._num_devices)
                for _ in range(self._num_devices)
            ]
            self._trajectories = jax.device_put_sharded(
                self._trajectories, jax.local_devices())
        ...
        act_params = jax.tree_map(lambda t: t[0], self._params)
        self._rng_key, epinfos = self._evaluate_actor.evaluate(
            self._rng_key, act_params)
...
class Experiment_Minigrid(WandbTrainableMixin, tune.Trainable):
    ...
    def step(self):
        ...
        log = jax.tree_map(lambda t: t.item(), log)
        log = jax.device_get(log)
        log.update({
            'ups': self._log_interval / (time.time() - t0),
            'num_frames': self._num_frames,
